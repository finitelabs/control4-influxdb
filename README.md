...
- Devices that share a room and name are now labelled with their device id, so
  the picker no longer shows rows that look identical. Devices with no variables
  are hidden when mapping a field or tag, and remain available under Device
  Tags.

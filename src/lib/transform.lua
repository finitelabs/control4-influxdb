...
function Transform.eval(expression, rawValue)
  ...
  -- Clone per-call to avoid mutating the cached function's environment.
  -- setfenv mutates in-place, so reusing the cached fn directly could
  -- cause issues if a C4 timer callback fires between setfenv and pcall.
  local sandboxed = function()
    return fn()
  end
  setfenv(sandboxed, env)

  local ok, result = pcall(sandboxed)
  if not ok then

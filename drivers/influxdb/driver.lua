...
--- Uses the InfluxWriter batch engine for buffering and flushing.
...
local function enqueueMeasurementPoint(measName)
  ...
  if not influxWriter then
    log:debug("enqueuePoint(%s): influxWriter not initialized yet, skipping", measName)
    return
  end

  -- Build fields table in InfluxWriter format: { [key] = { value = v, type = t } }
  local fields = {}
  for _, varIdStr in ipairs(meas.fields or {}) do
    ...
    if devId and varId then
      ...
      -- Auto-detect type: try number first, then boolean, then string
      local val = cached.value
      local valType
      if tonumber(val) then
        -- Use float to preserve decimal precision
        valType = constants.VALUE_TYPES.FLOAT
      elseif val and (val:lower() == "true" or val:lower() == "false") then
        valType = constants.VALUE_TYPES.BOOLEAN
      else
        valType = constants.VALUE_TYPES.STRING
      end
      fields[varName] = { value = val, type = valType }
    end
  end
  ...
  local intervalSecs
  local intervalStr = meas.interval or "Default"
  if intervalStr == "Default" then
    intervalSecs = config.writeInterval
  else
    intervalSecs = constants.WRITE_INTERVALS[intervalStr] or config.writeInterval
  end

  -- Enqueue via InfluxWriter batch engine
